-- TRINCA RV21 - Reset Brain foundation to canonical schema
-- Use only when the provisional brain tables are empty and can be safely recreated.

begin;

drop table if exists public.instagram_metrics cascade;
drop table if exists public.content_performance cascade;
drop table if exists public.ads_metrics cascade;
